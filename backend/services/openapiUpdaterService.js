...
export async function regenerateOpenApiWithSummary({ repoPath, targetUrl }) {
  ...
  try {
    ...
    if (candidates.length === 0) {
      result.error =
        "No candidate files with API routes found in the repository";
      return result;
    }
...
`.trim();
    ...
    console.log(
      `[OpenApiUpdater] Validation: ${validationResult.ok ? "valid" : "invalid"}`,
    );
    if (!validationResult.ok && validationResult.error) {
      console.error(
        `[OpenApiUpdater] Validation error: ${validationResult.error}`,
      );
    }
